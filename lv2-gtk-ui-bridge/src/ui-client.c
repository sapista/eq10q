...
int main(int argc, char* argv[])
{
    ...
    //Force all GTK2 child widgets (DrawingAreas, EventBoxes, Frames) to use same background as eq10q
    gtk_rc_parse_string(
        "style \"dark_ui_bridge\" {\n"
        "  bg[NORMAL]      = \"#303038\"\n"
        "  bg[ACTIVE]      = \"#303038\"\n"
        "  bg[PRELIGHT]    = \"#303038\"\n"
        "  bg[INSENSITIVE] = \"#303038\"\n"
        "  base[NORMAL]    = \"#303038\"\n"
        "  base[ACTIVE]    = \"#303038\"\n"
        "  base[PRELIGHT]  = \"#303038\"\n"
        "  base[INSENSITIVE] = \"#303038\"\n"
        "}\n"
        "widget_class \"*\" style \"dark_ui_bridge\"\n"
    );
    ...
    // Clean up plugin UI instance FIRST while IPC is still valid
    if (bridge.uihandle != NULL)
    {
        bridge.uiobj->desc->cleanup(bridge.uihandle);
        bridge.uihandle = NULL;
    }
